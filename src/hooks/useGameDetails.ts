...
const useGameDetails = (gameName: string) => {
    const fetchGameDetails = () => {
        return apiService.post<GameDetails[]>(
            '/games', 
            `fields name,summary,videos,screenshots,involved_companies,platforms,game_modes,themes,storyline,aggregated_rating,rating,total_rating,first_release_date;where slug="${gameName}";`).then(res => res.data[0])
    }
